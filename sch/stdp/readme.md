...
pot}(t) at time $t$. Here, $\delta t = t_{post}-t_{pre}$ is the difference between the time $t_{pre}$, which is the timing of the pre-synaptic spike, and $t_{post}$, which is the timing of the post-synaptic spike. When $\Delta t < 0 (\Delta t > 0), V_W$ is decreased (increased) by the depression (potentiation) circuit. The variation of $V_W, \delta V_W$, corresponds to the change in the synaptic weight of the pre- and post- STDP operations. 
